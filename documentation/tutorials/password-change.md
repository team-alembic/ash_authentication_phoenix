...
```
`update/2` is covered in the `Phoenix.LiveComponent` life-cycle documentation, so we won't go into it here.  The private function `assign_form/1` should look familiar if you have any forms for Ash resources in your application, but with a significant addition: the `prepare_source` option.  The `@current_user` assign set by AshAuthentication will not contain the value of the `hashed_password` attribute, because it is a private field.  This is what you normally want in your application, but the `:change_password` action needs this to validate the `:current_password` argument, so we need to explicitly load this attribute.  We also pass a private context field to the load operation to use in a field policy that we will write later on.
...
We also need to allow `ChangePasswordComponent` to read the `hashed_password` field, as we mentioned earlier.  We need to write a custom check to read the private context we set in the component earlier.
...
```elixir
  use Ash.Resource,
    # ...
    simple_notifiers: [Paddy.Notifiers.EmailNotifier]
```
```elixir
# lib/my_app/notifiers/email_notifier.ex
defmodule Paddy.Notifiers.EmailNotifier do
  use Ash.Notifier
  alias MyApp.Accounts.User
